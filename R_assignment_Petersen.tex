...
\section{To start I imported both the fang\_et\_al\_genotypes and
snp\_position into
...
\section{Type of tells you the type of vector the data is. In this case
it is list for both data
...
\section{To determine the type of list we can use the class
...
\section{Length shows the length of the data
...
\section{dim shows the dimensions of the data frame: number of rows and
number of
...
\section{The names of the columns are shown below only for the
snp\_positions.}\label{the-names-of-the-columns-are-shown-below-only-for-the-snp_positions.}

\section{Because there were 986 column names I did not list them all
here but the first three are listed
below}\label{because-there-were-986-column-names-i-did-not-list-them-all-here-but-the-first-three-are-listed-below}

\section{Sample\_ID, JG\_OTU, Group}\label{sample_id-jg_otu-group}
...
\section{The size of the files}\label{the-size-of-the-files}
...
\section{I used the command glimpse instead of str to look at the data
because the glimpse output is cleaner and easier to
...
\section{This shows a neat view of the data in the
files}\label{this-shows-a-neat-view-of-the-data-in-the-files}

\section{To observe the data delete
eval=FALSE}\label{to-observe-the-data-delete-evalfalse}
...
\section{Also because we are working in R, once the data has been
...
\section{This also helps with understanding what the variables in the
data
sets}\label{this-also-helps-with-understanding-what-the-variables-in-the-data-sets}

\section{Below I have described the variables for each of the data sets
that is important for the
assignment}\label{below-i-have-described-the-variables-for-each-of-the-data-sets-that-is-important-for-the-assignment}

\section{fang\_et\_al\_genotypes:}\label{fang_et_al_genotypes}

\section{Group --\textgreater{} This tells you what organism that data
came from. for our assignment we are intersted
in:}\label{group-this-tells-you-what-organism-that-data-came-from.-for-our-assignment-we-are-intersted-in}

\section{Maize group --\textgreater{} ZMMIL, ZMMLR, and
ZMMMR}\label{maize-group-zmmil-zmmlr-and-zmmmr}

\section{Teosinte group --\textgreater{} ZMPBA, ZMPIL, and
ZMPJA}\label{teosinte-group-zmpba-zmpil-and-zmpja}

\section{The rest of the column titles are the SNP\_IDs with the
...
\section{snp\_position:}\label{snp_position}

\section{SNP\_ID --\textgreater{} This is the snp
identifier}\label{snp_id-this-is-the-snp-identifier}

\section{Chromosome --\textgreater{} This is the chromosome on which the
SNP is
located}\label{chromosome-this-is-the-chromosome-on-which-the-snp-is-located}

\section{Position --\textgreater{} This is the location (bp) of the SNP
on that
chromosome}\label{position-this-is-the-location-bp-of-the-snp-on-that-chromosome}

\subsection{Data Processing}\label{data-processing}

\section{First I wanted to pull out the Maize and Teosinte groups from
the fang\_et\_al\_genotypes data
frame}\label{first-i-wanted-to-pull-out-the-maize-and-teosinte-groups-from-the-fang_et_al_genotypes-data-frame}

\section{I created new vectors titled Maize genotypes that contained the
groups ZMMIL, ZMMLR, and ZMMMR and Teosinte\_genotypes that contained
the groups ZMPBA, ZMPIL, and
...
\section{Next I wanted to transpose the mazie and teosinte vectors so
they could be joined with the SNP\_positions
data}\label{next-i-wanted-to-transpose-the-mazie-and-teosinte-vectors-so-they-could-be-joined-with-the-snp_positions-data}

\section{This created two new files, Mazie\_genotype\_tran and
...
\section{I was running into issues with manipulating the
...
\section{I used the the typeof() function to determine what type of
vector my new data
was}\label{i-used-the-the-typeof-function-to-determine-what-type-of-vector-my-new-data-was}

\section{Using these commands I determined that after using the
...
\section{I used the as.data.frame() function to change my vectors back
to data frames for easier
...
\section{These steps are to fix the column names so they can be merged
with the snp\_positions
...
\section{Now I need to adjust the row names for the maize and teosinte
data frames so Sample\_ID is a row name with the correct data in the
column. This will be needed to merge the Maize and Teosinte data with
the snp\_position
...
\section{I used the mutate function to add a new column to the data
frame. This moved the sample\_ID names from being row titles to being
data in the data
...
\section{Cutting down the snp\_position files to have only the data I
...
\section{I used the Merge function to merge the snp\_position data with
the Maize and Teosinte
data.}\label{i-used-the-merge-function-to-merge-the-snp_position-data-with-the-maize-and-teosinte-data.}

\section{I merged the data by the sample\_ID column from the Maize data
and the snp\_ID column from the snp\_positions
...
\section{I also made sure to keep the other columns in the
...
\section{To create a function to iterate over all the chromosomes I
wanted to see if both maize and teosinte had the same number of unique
...
\section{If the vectors weren't the same length then the iteration in
the next step wouldn't
work}\label{if-the-vectors-werent-the-same-length-then-the-iteration-in-the-next-step-wouldnt-work}

\section{I created a vector with all the unique maize and teosinte
...
\section{I am using an if else statement to compare the lengths of the
vectors.}\label{i-am-using-an-if-else-statement-to-compare-the-lengths-of-the-vectors.}

\section{The addition of the sort function orders the chromosomes so
they can be compared evenly. !identical means not
...
\section{``Maize and Teosinte vectors are equal'' was printed so I can
continue to the function and iteration
...
\section{Writing a function to be able to iterate the formation of the
40 necessary
files}\label{writing-a-function-to-be-able-to-iterate-the-formation-of-the-40-necessary-files}

\section{Create\_files is the main function that I will use for lapply
further
down.}\label{create_files-is-the-main-function-that-i-will-use-for-lapply-further-down.}

\section{Iterate\_chr is the function that is doing the majority of the
...
\section{The data\_sorted is going to filter out only the chromosome
column that matches the chr that is currently being iterated
...
\section{Then it is going to arrange by position by ascending or
...
\section{Essentially if the logic returns a TRUE then it will sorted in
descending order. If the logic returns a FALSE then it will sort in
ascending
...
\section{Then using the mutate command and across(everything()) command
I was able to change the ? to - for the decreasing position
...
\section{The \textbackslash? allows the ? to act like a character and
not a ``special character'' in
R}\label{the-allows-the-to-act-like-a-character-and-not-a-special-character-in-r}

\section{Now I am using the Iterate\_chr function to create 4 different
...
\section{Using the write\_csv command I am taking the four file versions
created above and saving them as each of the 40 files needed with the
chr number changing depending on which chromosome is being worked on by
the
...
\section{The paste0 command just combines the naming convention in the
parenthesis}\label{the-paste0-command-just-combines-the-naming-convention-in-the-parenthesis}

\section{Finally I used lapply to run Create\_files for all the
...
\section{The output of the lapply is the 40 individual CSV
files}\label{the-output-of-the-lapply-is-the-40-individual-csv-files}
...
\section{Putting the data tables in the long format instead of the wide
format helped To make it tidy and easier to
...
\section{Using the pivot\_longer command it flipped the data so all of
the sample\_IDs are in one column with their respective SNP and
...
\section{I has to use the mutate(across(everything)), as.character() to
tell R to treat the entire data table as a character because previosuly
there was a mix of characters and integers and R doesn't like
...
\section{I also kept the the chromosome and position columns when
flipping the data
...
\section{SNP Distribution by
Chromosome}\label{snp-distribution-by-chromosome}

\section{To compare the difference in the number of SNPs across
...
\section{I created a data table with the number of SNPs per chromosome
and then sorted that data table by chromosome
...
\section{I combined the SNPs count data tables for Maize and Teosinte by
adding a species column. Then combined them using the rbind
...
\section{Then I again sorted them by chromosome
...
\section{I used ggplot to create the double bar chart. I used stat =
...
\subsection{With the bar chart you can see that Maize has more SNPs per
chromosome than Teosinte and the larger the larger the chromosome number
the less amount of SNPs
present}\label{with-the-bar-chart-you-can-see-that-maize-has-more-snps-per-chromosome-than-teosinte-and-the-larger-the-larger-the-chromosome-number-the-less-amount-of-snps-present}
...
\section{I needed to change the poisiton values to numerical data in R
so it could be
...
\section{I used ploted both Maize and Teosinte on teh plots again this
time
overlapped}\label{i-used-ploted-both-maize-and-teosinte-on-teh-plots-again-this-time-overlapped}

\section{I used the facet wrap command to that all the chromosomes had a
different plot but where all still on the same axis for easy
...
\subsection{Again we see that maize has more SNPs than Teosinte.
Chromosome 7 is also interesting becuase there are no SNPs in the middle
of the
chromosome.}\label{again-we-see-that-maize-has-more-snps-than-teosinte.-chromosome-7-is-also-interesting-becuase-there-are-no-snps-in-the-middle-of-the-chromosome.}
...
\section{I created a new column that was labeled either Missing,
...
\section{I plotted the resulting data in a stacked bar chart to compare
Maize and
Teosinte.}\label{i-plotted-the-resulting-data-in-a-stacked-bar-chart-to-compare-maize-and-teosinte.}

\subsection{While the highest proportions of SNPs came from homozygous
genotypes, Teosinte has a higher proportion of heterozygous genotypes
than
Maize}\label{while-the-highest-proportions-of-snps-came-from-homozygous-genotypes-teosinte-has-a-higher-proportion-of-heterozygous-genotypes-than-maize}
...
\section{Distribution of Genotype Type by
Chromosome}\label{distribution-of-genotype-type-by-chromosome}

\section{I plotted the proportion of each type of SNP (heterozygous,
homozygous, or Missing) for each chromosome for both Maize and
...
\section{This will show is there are any chromosomes that are extremely
different in terms of the type of SNP for either Teosinte or
...
\subsection{Based on the plots there was not any discernible trends that
would warrant further
investigation}\label{based-on-the-plots-there-was-not-any-discernible-trends-that-would-warrant-further-investigation}
