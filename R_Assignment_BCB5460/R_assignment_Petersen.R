...
# I made sure that both had the headings option set to yes
...
# The names of the columns are shown below only for the snp_positions. 
...
# Because there were 986 column names I did not list them all here but the first three are listed below
# Sample_ID, JG_OTU, Group

# The size of the files 
object.size(fang_et_al_genotypes) # 22681376 bytes
object.size(snp_position) # 327392 bytes

# I used the command glimpse instead of str to look at the data because the glimpse output is cleaner and easier to read.
# This shows a neat view of the data in the files
...
# This also helps with understanding what the variables in the data sets
# Below I have described the variables for each of the data sets that is important for the assignment 
...
# First I wanted to pull out the Maize and Teosinte groups from the fang_et_al_genotypes data frame
# I created new vectors titled Maize genotypes that contained the groups ZMMIL, ZMMLR, and ZMMMR and Teosinte_genotypes that contained the groups ZMPBA, ZMPIL, and ZMPJA
...
# Next I wanted to transpose the mazie and teosinte vectors so they could be joined with the SNP_positions data
# This created two new files, Mazie_genotype_tran and Teosinte_genotype_tran
Maize_genotype_tran <- t(Maize_genotype)
Teosinte_genotype_tran <- t(Teosinte_genotype)

# I was running into issues with manipulating the Maize_genotype_tran and Teosinte_genotype_tran
# I used the the typeof() function to determine what type of vector my new data was
typeof(Maize_genotype_tran)
typeof(Teosinte_genotype_tran)
# Using these commands I determined that after using the transpose function the data is converted to a character vector
...
# Now I need to adjust the row names for the maize and teosinte data frames so Sample_ID is a row name with the correct data in the column. This will be needed to merge the Maize and Teosinte data with the snp_position data 
# I used the mutate function to add a new column to the data frame. This moved the sample_ID names from being row titles to being data in the data frame
...
# I used the Merge function to merge the snp_position data with the Maize and Teosinte data. 
# I merged the data by the sample_ID column from the Maize data and the snp_ID column from the snp_positions data. 
# I also made sure to keep the other columns in the snp_positions data because I need the chromosome number later on
...
# I am using an if else statement to compare the lengths of the vectors. 
# The addition of the sort function orders the chromosomes so they can be compared evenly. !identical means not identical
...
# Writing a function to be able to iterate the formation of the 40 necessary files
# Create_files is the main function that I will use for lapply further down. 
# Iterate_chr is the function that is doing the majority of the work for the iteration process
...
# The data_sorted is going to filter out only the chromosome column that matches the chr that is currently being iterated over
# Then it is going to arrange by position by ascending or descending values. I used the desc=FALSE to aid in the process
  # Essentially if the logic returns a TRUE then it will sorted in descending order. If the logic returns a FALSE then it will sort in ascending order 
# Then using the mutate command and across(everything()) command I was able to change the ? to - for the decreasing position files
  # The \\? allows the ? to act like a character and not a "special character" in R
  
# Now I am using the Iterate_chr function to create 4 different categories of the sorted data
...
# Using the write_csv command I am taking the four file versions created above and saving them as each of the 40 files needed with the chr number changing depending on which chromosome is being worked on by the function
# The paste0 command just combines the naming convention in the parenthesis
...
# Putting the data tables in the long format instead of the wide format helped To make it tidy and easier to plot
# Using the pivot_longer command it flipped the data so all of the sample_IDs are in one column with their respective SNP and genotypes
...
# I created a data table with the number of SNPs per chromosome and then sorted that data table by chromosome number 
...
# I combined the SNPs count data tables for Maize and Teosinte by adding a species column. Then combined them using the rbind command.
# Then I again sorted them by chromosome number 
...
# I used ggplot to create the double bar chart. I used stat = identity so that it uses the count that is already done. I also used position = dodge to put the bar next to each other instead of overlapping
...
## With the bar chart you can see that Maize has more SNPs per chromosome than Teosinte and the larger the larger the chromosome number the less amount of SNPs present
...
# I used ploted both Maize and Teosinte on teh plots again this time overlapped
# I used the facet wrap command to that all the chromosomes had a different plot but where all still on the same axis for easy comparison
...
Combined_long$Type <- ifelse(Combined_long$Genotype %in% c("?/?"), "Missing",
                      ...
                      "Heterozygous"))
# I plotted the resulting data in a stacked bar chart to compare Maize and Teosinte. 
...
## While the highest proportions of SNPs came from homozygous genotypes, Teosinte has a higher proportion of heterozygous genotypes than Maize

# Distribution of Genotype Type by Chromosome
# I plotted the proportion of each type of SNP (heterozygous, homozygous, or Missing) for each chromosome for both Maize and Teosinte
# This will show is there are any chromosomes that are extremely different in terms of the type of SNP for either Teosinte or Maize
...
## Based on the plots there was not any discernible trends that would warrant further investigation 
