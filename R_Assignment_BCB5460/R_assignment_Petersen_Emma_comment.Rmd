...
# Comment: This "glimpse" function is slow and heavy
...
### Finally I used lapply to run Create_files for all the chromosomes. Using the invisible command prevents lapply from printing the results on to the console. It keeps things looking cleaner. **The output of the lapply is the 40 individual CSV files**
```{r}
...
ggplot(Combined_snp_count, aes(x = Chromosome, y = SNPs, fill = Species)) + 
  ...
  theme( # Comment: A way to make the x-axis easier to read?
...
# # Comment: Cleaner plot?
# ggplot(Combined_long, aes(x = Position, fill = Species)) +
#   geom_histogram(binwidth = 5e6, position = "identity", alpha = 0.6) +
#   facet_wrap(~Chromosome, ncol = 5, scales = "fixed") +
#   labs(
#     title = "SNP Distribution per Chromosome",
#     x = "Position (Mb)",
#     y = "Number of SNPs",
#     fill = "Species"
#   ) +
...
#   )) +
#   scale_x_continuous(
#     limits = c(0, max(Combined_long$Position, na.rm = TRUE)),
